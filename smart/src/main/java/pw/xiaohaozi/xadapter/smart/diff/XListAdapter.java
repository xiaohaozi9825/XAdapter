 ...
 * <p>
 * This class is a convenience wrapper around {@link XAsyncListDiffer} that implements Adapter common
 ...
 * class should refer to {@link XAsyncListDiffer}, which provides custom mapping from diff events
 * to adapter positions.
 *
 * @param <T>  Type of the Lists this Adapter will receive.
 * @param <VH> A class that extends ViewHolder that will be used by the adapter.
 */
public abstract class XListAdapter<T, VH extends RecyclerView.ViewHolder>
        extends RecyclerView.Adapter<VH> {
    final XAsyncListDiffer<T> mDiffer;
    private final XAsyncListDiffer.ListListener<T> mListener =
            new XAsyncListDiffer.ListListener<T>() {
                @Override
                public void onCurrentListChanged(
                        @NonNull List<T> previousList, @NonNull List<T> currentList) {
                    XListAdapter.this.onCurrentListChanged(previousList, currentList);
                }
            };

    @SuppressWarnings("unused")
    protected XListAdapter(@NonNull DiffUtil.ItemCallback<T> diffCallback) {
        mDiffer = new XAsyncListDiffer<T>(new AdapterListUpdateCallback(this),
                new XAsyncDifferConfig.Builder<T>(diffCallback).build());
        mDiffer.addListListener(mListener);
    }

    @SuppressWarnings("unused")
    protected XListAdapter(@NonNull XAsyncDifferConfig<T> config) {
        mDiffer = new XAsyncListDiffer<T>(new AdapterListUpdateCallback(this), config);
        mDiffer.addListListener(mListener);
    }
