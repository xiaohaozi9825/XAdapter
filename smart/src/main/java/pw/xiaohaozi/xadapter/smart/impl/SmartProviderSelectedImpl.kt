...
open class SmartProviderSelectedImpl<Employer : XProxy<Employer>, VB : ViewBinding, D> :
    ...
    private val provider: XProvider<VB, D> by lazy {
        when (val e = employer) {
            is XProvider<*, *> -> e as XProvider<VB, D>
...
//    }
    ...
    open protected fun cancelCheck(
        data: D, holder: XHolder<VB>?, view: View?
    ) {
        //必须在删除前拿到被删除item索引
        val indexOf = selectedCache.indexOf(data)
    ...
    open protected fun clickCheck(
        holder: XHolder<VB>?, isCheck: Boolean, position: Int, view: View?
    ) {
    ...
    open protected fun notifyCheckChanges(
        holder: XHolder<VB>?, data: D, position: Int, index: Int, view: View?
    ) {
