 ...
 * 单布局
 */
class EditNodeFragment : VBFragment<FragmentNodeEditBinding>() {
    ...
    private fun NodeProvider<ViewBinding, NodeEntity<*, *>, ItemNodeEditBinding, NodeInfo2>.editChild(data: NodeInfo2) {
        InputDialog(requireActivity())
            .setTitle("编辑内容")
            .setMsg(data.text)
            .onConfirm { _, content: String ->
                val newNode = NodeInfo2(data.no, content)
