...
### 进阶用法
```kotlin
//当我们逻辑比较复杂时，我们可以单独创建一个Provider类，在该类中实现ViewHolder初始化与绑定工作
...
//创建并使用
val adapter = SingleProvider().adapter
```

## 示例代码

item_verse_data_binding.xml
...
class SingleFragment : VBFragment<FragmentRecyclerBinding>() {
    //①创建adapter
    private val adapter = createAdapter<ItemVerseDataBindingBinding, VerseInfo> {
        it.binding.data = it.data 
    }
