# 创建多布局Adapter
> 快速创建多种布局的Adapter。不同布局，可以根据数据类型自动生成itemType，无需多余的分支语句判断itemType。
## 用法
...
### 基础用法
```kotlin
//①如果多个布局使用的数据类型均不相同，则会根据数据类型自动判断itemType，无需手动指定。
...
val adapter = createAdapter()
    ...
    .withType<ItemImageCardBinding, MultipleInt>(itemType = 8) {
        //实现绑定逻辑
    }
    .toAdapter()

//③如果多个布局使用的数据类型相同，则需要在withType中指定类型，并在createAdapter方法中动态返回item对应的itemType。
val adapter = createAdapter { data, position ->
        return if ((data as Int) % 2 == 0 ) 1
        else 2
    }.withType<ItemVerseBinding, Int>(itemType = 1) { 
        //实现绑定逻辑
    }.withType<ItemImageCardBinding, Int>(itemType = 2) { 
        //实现绑定逻辑
    }.toAdapter()
...
//③创建adapter并组合多个Provider
fun createAdapter():SmartAdapter<ViewBinding, Any?>{
    //①创建Adapter
    ...
    //方式二：使用➕链接，viewType为空
    return xAdapter + provider1 + provider2
}
```
...
data class VerseInfo(
    var content: String,//诗句内容
    var author: String//作者or出处
)
```
MultipleFragment.kt
