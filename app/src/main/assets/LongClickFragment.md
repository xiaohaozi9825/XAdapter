...
fun setOnLongClickListener(
    id: Int? = null, 
    listener: OnItemLongClickListener<Employer, VB, D>
): Employer
...
```kotlin
//为adapter设置长按事件监听
val adapter = createAdapter<ItemVerseBinding, VerseInfo> {(holder,data,position)->
        //实现绑定逻辑
...
class ClickFragment : VBFragment<FragmentRecyclerBinding>() {
    ...
    val adapter = createAdapter()
        ...
        }.setOnLongClickListener { holder, data, position, view ->//设置当前布局长按事件
        ...
        .toAdapter()
        .setOnLongClickListener { holder, data, position, view ->//设置全局长按事件
